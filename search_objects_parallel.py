...
#############################################################################
# SEARCH_OBJECTS_PARALLEL.py performs a conesearch around a given object and retrieves  
...
def search_catalogs(*args):
    ...
    begin, end, catalog_name, name, position, radius = int(args[0]), int(args[1]), args[2], args[3], args[4], args[5] #length of args = 6
    #search_catalogs(begin, end, catalogs[k], names[k], converted_str, radius)
...
def main():
    ...
    for k in xrange(m):
        ...
        writeflag, table_per_catalog = search_catalogs(begin, end, catalogs[k], names[k], converted_str, radius)
